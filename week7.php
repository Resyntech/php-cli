<?php
    ...
    $months = [];
    $title = 'Week 7 | Looping - Applicable to all years without February 29'. PHP_EOL;
    $continue = true;
    $wrongChoice = false;
    $minMonth = 28;
    $midMonth = 30;
    ...
    $index = 1;
    for ($index; $index <= 7; $index++) { // First 7 Months
        if ($index === 2 && true) // Month of February
    ...
    $index = 1;
    for ($index; $index <= 5; $index++) { // Last 5 Months
        if ($index === 2 && false) // Month of February
            array_push($months, $minMonth);
        else
            switch($index % 2) {
                case 0: // Even Month
